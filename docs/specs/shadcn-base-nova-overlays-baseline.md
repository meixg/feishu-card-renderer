...
`pnpm ui:verify` 会按 manifest/component 分组精确对照独立的
[`ui-provenance-expected.mjs`](../../scripts/ui-provenance-expected.mjs) 可信 registry，
而非仅校验 hash 格式或以 manifest 自身作为 expected source。该 registry 同时
固定 upstream blob 与 reviewed local adaptation：manifest 的 `localFiles` key
集合必须完全相等，每个 manifest hash 与实际本地文件 hash 都必须等于独立
reviewed hash。当前覆盖 Button、form controls、overlays、containers 和
Calendar/Popover。可信 expected 不从 manifest 生成、不访问网络，并只哈希组件
或 scoped CSS，不哈希共享 `src/styles.css`，避免无关 shotgun drift。
