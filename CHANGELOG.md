...
- 9441f2e: 将 table 翻页器对齐 shadcn Pagination 的标准页码、活跃态、省略号及前后翻页布局，并支持直接跳页。
- 9bd892e: 将 table 翻页器调整为居右布局，并消除页码及前后翻页按钮按下时产生的瞬时滚动条。
...
- 9e860d8: 重置图表 `figure` 的浏览器默认外边距，避免图表宽度溢出卡片容器。
- 9015f35: 修复独立 Markdown 组件将段落内单个换行符折叠为空格的问题，使多行内容按可见 soft break 渲染，消费者无需修改现有卡片 JSON。
- 0399be0: 修复多个交互提示在可访问描述中连写的问题，使读屏软件能清楚区分悬停说明和禁用原因。
