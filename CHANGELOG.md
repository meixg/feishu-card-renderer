...
- 95f09b7: 升级内置 Lucide 图标资源至 1.28.0，日期选择器的日历图标细节随上游版本更新。
- b15c1c8: 修复折叠面板未配置 `header.icon` 时仍显示默认箭头并应用图标位置布局的问题。
- cbb765d: 修复彩色 Header 模板顶部未继承卡片圆角的问题。

## 0.1.3

### Patch Changes

- fc81ac7: 修复卡片标题未应用 `header.template` 主题色的问题，并为非法主题值提供诊断和默认样式降级；消费者无需修改现有卡片 JSON。
- 961a6d2: 为必填表单控件的标签显示红色星号，并在空值提交时保留前端校验与无障碍必填语义，消费者无需修改现有卡片配置。
